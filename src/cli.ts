        ...
        await Promise.all(
          chunk.map((file) =>
            this.convertSingleFile(
              ...
              path.join(
                outputDir,
                this.getOutputFileName(file, conversionOptions.outputFormat || 'plain')
              ),
              conversionOptions,
              options.verbose
            )
    ...
    try {
      ...
      if (result.parseResult.errors.length > 0) {
        ...
        result.parseResult.errors.forEach((error, index) => {
          console.log(
            `  ${index + 1}. ${error.message}${error.line ? ` (Line ${error.line})` : ''}`
          );
        });
      }
      ...
      if (result.parseResult.warnings.length > 0) {
        ...
        result.parseResult.warnings.forEach((warning, index) => {
          console.log(
            `  ${index + 1}. ${warning.message}${warning.line ? ` (Line ${warning.line})` : ''}`
          );
        });
      }
  ...
  private async convertDirectory(
    ...
    for (const file of files) {
      ...
      const outputPath = path.join(
        output,
        this.getOutputFileName(relativePath, options.outputFormat || 'plain')
      );
  ...
  private async watchFiles(
    ...
    watcher.on('change', async (filePath) => {
      if (path.extname(filePath) === '.py') {
        ...
        try {
          const output =
            outputPath || this.getOutputFileName(filePath, options.outputFormat || 'plain');
